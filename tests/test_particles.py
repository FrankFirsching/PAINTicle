...
def sim_only(setup_info):
    for i in range(100):
        ...
        setup_info.particles.move_particles(setup_info.settings.physics, setup_info.delta_t)
...
def sim_setup():
    ...
    return SetupInfo(test_mesh, context, particles, 0.01, bpy.context.scene.painticle_settings,
                     tstutils.get_fake_event(x=context.region.width//2, y=context.region.height//2))


def test_benchmark_no_painting(benchmark, sim_setup):
    benchmark.pedantic(sim_only, args=(sim_setup,), rounds=1)


@pytest.mark.skipif(tstutils.no_ui(), reason="requires UI")
def test_benchmark_painting(benchmark, sim_setup):
